...
function M.pick_tag_references()
  ...
  for _, entry in ipairs(occurrences) do
  ...
  picker.show({
    title = "Tag References: " .. uuid:sub(1, 16) .. "...",
    items = entries,
    on_select = function(item)
      -- Open file and jump to line
      vim.cmd(string.format("edit +%d %s", item.line_num, item.filepath))
    end,
    preview = {
      type = 'file',
